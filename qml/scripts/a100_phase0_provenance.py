    ...
    with torch.no_grad():
        for imgs, _tab, targets in loader:
            ...
            try:
                ...
                # HybridQuantumClassifier.forward() doesn't accept return_logits
                out = model(imgs)
            # CornealEncoder returns (latent, logits); HybridQuantumClassifier returns logits directly
...
def phase_0c(device: torch.device) -> dict:
    ...
    if hybrid_ckpt.exists():
        ...
        try:
            ...
            state = torch.load(str(hybrid_ckpt), map_location=device, weights_only=True)
            info = hybrid.load_state_dict(state, strict=False)
            if info.missing_keys:
                logger.info("Hybrid load (strict=False) missing keys (expected for supervised_head): %s", info.missing_keys)
...
def main():
    ...
    # Log to MLflow (best-effort — may fail if disk is full)
    try:
        ...
        logger.warning("MLflow logging failed (disk full?): %s", e)
        print("[MLflow] Skipped — results already saved to JSON.")
